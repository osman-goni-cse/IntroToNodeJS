...
const server = http.createServer(function (req, res) {
  if (req.method === 'GET') {
    ...
    stream.on('end', () => {
      writeToLogFile(req, res, "File Reading Completed");
      res.end();
    });
    ...
    stream.on('error', (err) => {
      if (err.code == 'ENOENT') {
        ...
        writeToLogFile(req, res, "File Not Found");
      } else if (err.code == 'EACCES') {
        res.statusCode = 403;
        res.end('Permission denied');
        writeToLogFile(req, res, "Permission Denied");
      }
      else if(err.code == 'EPERM') {
        res.statusCode = 501;
        res.end("Don't Have Permission");
        writeToLogFile(req, res, "Don't have Permission");
      } 
      else {
        console.error(err);
        res.statusCode = 500;
        res.end('Internal server error');
        writeToLogFile(req, res, "Internal Server Error");
      }
    });
  }
  else {
